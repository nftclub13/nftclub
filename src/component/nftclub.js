...
class NftClub extends Component {
    ...
    invest() {
        ...
        // abi.invest(account.pk, account.mainPKr, value, code)
    }
    ...
    render() {
        ...
        let items = this.state.details.achievements.map(function (value, index) {
                ...
                return (
                    <List.Item key={index}
                               style={{borderRadius: (index == self.state.details.achievements.length - 1) ? '0 0 5px 5px' : ''}}>
                ...
                <WingBlank size="lg">
                    <List renderHeader={<span className="title">{language.e().account.recommend.title}</span>}>
                        ...
                        <div className="item-header"
                             style={{borderRadius: (self.state.details.achievements.length === 0) ? '0 0 5px 5px' : ''}}>
                            <div style={{float: "left", width: '25%', textAlign: 'center'}}><span
